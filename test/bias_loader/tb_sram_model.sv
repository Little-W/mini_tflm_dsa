  ...
  always_ff @(posedge clk or negedge rst_n) begin
    if (!rst_n) begin
      ...
      if (cmd_valid && cmd_ready) begin
        automatic logic [ADDR_W-1:0] word_idx;
        word_idx = cmd_addr >> $clog2(DATA_W/8);
        rsp_valid_r <= 1'b1;
        if (cmd_read) begin
          rsp_rdata_r <= (word_idx < DEPTH) ? mem[word_idx] : {DATA_W{1'b1}};
        end else begin
          if (word_idx < DEPTH) begin
            for (int b = 0; b < DATA_W/8; b++) begin
              if (cmd_wmask[b]) begin
                mem[word_idx][b*8 +: 8] <= cmd_wdata[b*8 +: 8];
              end
            end
          end
          rsp_rdata_r <= '0;
